...
If a close call is detected (that is, the IR Distance sensor is tripped), optionally, data can be stored using the Intel® IoT Examples data store or an MQTT server running in your own Microsoft\* Azure\*, IBM\* Bluemix\*, AT&T\* M2X\*, or AWS account.
